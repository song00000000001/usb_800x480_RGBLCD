...
**当前 USB 模式**: Vendor 模式（Windows 专用驱动）
...
| 功能 | 状态 | 说明 |
|------|------|------|
| **Vendor 模式** | ✅ 启用 | Windows 专用数据传输，512字节帧头协议 |
| **RGB LCD 显示** | ✅ 启用 | 800×480@30MHz，支持RGB565/JPEG格式 |
| **ES8388 音频** | ✅ 启用 | I2S 接口，UAC 音频播放/录制 |
| **HID 接口** | ✅ 定义 | 已禁用触摸功能，仅保留结构体定义 |
| **UAC 音频** | ✅ 启用 | 24kHz采样率，单声道，16位宽，双向传输 |
| **帧缓冲管理** | ✅ 启用 | 8个帧缓冲，支持JPEG压缩传输 |

### 已禁用的功能

| 功能 | 状态 | 说明 |
|------|------|------|
| **HID 触摸上报** | ❌ 禁用 | 触摸数据已禁用，仅保留接口 |
...
│   ├── LCD/                        # LCD 驱动
│   │   ├── lcd.c/h                # LCD 通用驱动
│   │   ├── rgblcd.c/h             # RGB LCD 驱动
│   │   └── mipi_lcd.c/h           # MIPI LCD 驱动
│   ├── ES8388/                     # 音频 Codec
│   ├── MYI2S/                      # I2S 接口
│   ├── MYIIC/                      # I2C 接口
│   ├── XL9555/                     # IO 扩展芯片
...
├── screen_stream_portal.ts         # TypeScript 版投屏脚本
...
### screen_stream_portal.ts - TypeScript 版本

与 screen_stream_portal.py 功能相同的 TypeScript 实现：
```bash
npx ts-node screen_stream_portal.ts
```
...
- **ESP-IDF**: v6.1
- **目标芯片**: ESP32-P4
- **LCD 面板**: 4.3 寸 800×480 RGB LCD (ID: 0x4384)
- **Python**: 3.x
