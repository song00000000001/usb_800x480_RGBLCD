...
DRAM_ATTR void *lcd_buffer[1];              /* 指向屏幕单缓存 - 解决双缓冲区切换导致的随机偏移 */
 ...
 *              1 :1 :0     4.3 寸 800*400  RGB 屏，ID = 0X4350
 * 
 * @param       无
 * @retval      0, 非法; 
 *              其他, LCD ID
...
uint16_t lcd_panelid_read(void)
{
    ...
    switch (idx)
    {
        ...
        case 6:
        {
            return 0x4350;                      /* 4.3 寸 800*400 RGB 屏 */
        }
        default:
        {
            return 0;
...
void lcd_init(void)
{
    ...
    ESP_LOGI(TAG, "LCD Panel ID: 0x%04X", lcddev.id);
    ...
    {
        ESP_LOGI(TAG, "Using RGB LCD");
        lcddev.lcd_panel_handle = rgblcd_init();                /* 初始化RGB LCD */
        ESP_ERROR_CHECK(esp_lcd_rgb_panel_get_frame_buffer(lcddev.lcd_panel_handle, 1, &lcd_buffer[0]));
...
{
    uint16_t *buffer = (uint16_t *)lcd_buffer[0];  /* 使用单帧缓冲区 */
