...
bool tud_audio_rx_done_post_read_cb(uint8_t rhport, uint16_t n_bytes_received, uint8_t func_id, uint8_t ep_out, uint8_t cur_alt_setting)
{
     ...
     */
    if (now - last_time > 100 * UAC_SPK_NEW_PLAY_INTERVAL) {
        new_play = true;
...
esp_err_t uac_device_init(uac_device_config_t *config)
{
    ...
    if (!config->skip_phy_init) {
        usb_phy_init();
    }

#if CFG_TUD_AUDIO_FUNC_1_N_CHANNELS_RX
    BaseType_t ret_val = xTaskCreatePinnedToCore(usb_mic_task, "usb_mic_task", 4096, NULL, UAC_MIC_TASK_PRIORITY,
                                                 &s_uac_device->mic_task_handle, UAC_MIC_TASK_CORE == -1 ? tskNO_AFFINITY : UAC_MIC_TASK_CORE);
    ESP_RETURN_ON_FALSE(ret_val == pdPASS, ESP_FAIL, TAG, "Failed to create usb_mic task");
#endif

#if CFG_TUD_AUDIO_FUNC_1_N_CHANNELS_TX
    ret_val = xTaskCreatePinnedToCore(usb_spk_task, "usb_spk_task", 4096, NULL, UAC_SPK_TASK_PRIORITY,
                                      &s_uac_device->spk_task_handle, UAC_SPK_TASK_CORE == -1 ? tskNO_AFFINITY : UAC_SPK_TASK_CORE);
